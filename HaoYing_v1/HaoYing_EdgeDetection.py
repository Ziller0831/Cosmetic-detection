...
if __name__ == "__main__":
    ...
    if Mode == 0:  ##! 物件輪廓選擇mode
        initial = HY.initialize()
        area_size, area_mean, area_std = initial.AreaIdent(cap)
        HY.CSVDataWrite(Choose_product, Csv_Dir, area_size, area_mean, area_std)
        ...
        for i in calibration:
            HY.XMLWrite(r"Setting.xml", str(i),  [[float(num) for num in row] for row in calibration[str(i)]])
    
    elif Mode == 2: ##! working mode
        TF_para = {"camera_matrix": 0, "rvecs": 0, "tvecs": 0}
        for i in TF_para:
            TF_para[i] = HY.XMLRead(r"Setting.xml", str(i))

        vision = HY.EdgeDetector(Mode, Choose_product, TF_para)
        time_start = time()
