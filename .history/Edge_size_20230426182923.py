...
if __name__ == '__main__':
    ...
    while len(SizeData) < 100: 
        ...
        for contour in contours:
            ...
            SizeData.append(cv2.contourArea(contour))

        cv2.drawContours(frame, contours, -1, (0,0,255), 2)
        cv2.imshow("Result", frame)
        if cv2.waitKey(1) & 0xFF == 27: ## 27 = ESC
            break
    maxCont = max(SizeData)
    minCont = min(SizeData)
    meanCont = np.mean(np.array(SizeData, dtype=object))
    stdCont = np.std(np.array(SizeData,  dtype=float), axis=0, ddof=1)
    print('Max:', maxCont, 'Min:', minCont, "Mean:", meanCont, 'Std:', stdCont)
