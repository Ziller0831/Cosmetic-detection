...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            text = f"({str(gravity_point[0])}, {str(gravity_point[1])})"
        ...
        for min_rect in minRect_array:
            ...
            print(min_rect)
        time_spent = round(1/(time.time() - time_start))
        

        # cv2.circle(Result, raw_minRect[0], 2, (0,255,255), 2)
        cv2.putText(frame, str(time_spent), (10, 30), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 2)
