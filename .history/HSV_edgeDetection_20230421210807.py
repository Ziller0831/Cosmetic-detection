...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Result = np.mean(np.array(dataBuffer), 1)
        # print(Result)

        # resultData = [center_point[0], center_point[1], round(theta, 4)]
        diff_buffer = len(dataBuffer[0]) - int(fps//TargetFPS) + 1
        print(len(dataBuffer[0]) - int(fps//TargetFPS) + 1)
        dataBuffer = dataBuffer[diff_buffer if diff_buffer >= 0 else 0:]
        dataBuffer.append(resultData)

        # print(int(fps//TargetFPS), len(dataBuffer[0]), dataBuffer)
