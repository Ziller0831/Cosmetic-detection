...
TargetFPS = 5
AngleZero_offset = 90 ## * 0度為x軸正向 順時針範圍0~360
...
def AngleIndentify(deltaX, deltaY, angle):
    if deltaX > 0:
        return angle if deltaY < 0 else 0 if deltaY == 0 else (angle+270) if deltaY > 0 else None
    if deltaX < 0:
        return (angle+90) if deltaY < 0 else (angle+180) if deltaY > 0 else (angle+180) if deltaY == 0 else None
    if deltaX == 0:
        return (angle+90) if deltaY < 0 else (angle+270) if deltaY > 0 else None
...
if __name__ == '__main__':
    ...
    while True:
        ...
        np.zeros((len(contours), fps/TargetFPS), dtype=np.int8)
        for contour in contours:
            moment = cv2.moments(contour)
            gravity_point = np.array(GravityCalc(moment))
            center_point, minRect, theta = MinRectCircle(contour)

            deltaX, deltaY = gravity_point - center_point

            theta_1 = AngleIndentify(deltaX, deltaY, 90-theta)
            try:
                print("%3.2f, %3.2f"%(theta-90, theta_1-90), end = "\r")
            except:
                continue
        ...
        text = f"({str(gravity_point[0])}, {str(gravity_point[1])}, {theta_1-90:.2f})" ## fstring
        # text = "({0}, {1})".format(str(gravity_point[0]), str(gravity_point[1])) ##另一種打法
        cv2.putText(frame, text, (gravity_point[0]+10, gravity_point[1]+10), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 0), 2, 8, 0)
