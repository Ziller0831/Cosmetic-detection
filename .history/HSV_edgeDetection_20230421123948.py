...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            Centroid_point = Centroid(frame, moment)
            min_rect_array.append(MinRectCircle(contour))

            cv2.circle(frame, Centroid, 2, (0,0,255), 2)
            text = "(" + str(Centroid[0]) + ", " + str(Centroid[1]) + ")"
            cv2.putText(frame, text, (Centroid[0]+10, Centroid[1]+10), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 1, 8, 0);


        # minRect_array = MinRectCircle(contours)
        # Result, angle  = ContoursDetail(frame, contours)
