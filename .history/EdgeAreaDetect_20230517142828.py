...
class initialize:
    ...
    def ScaleCalc(self):
        ...
        while cv2.waitKey(1) & 0xFF != ord('q'):
            frame = vision.ImageCatch(cap)
            cv2.circle(frame, self.CheckPoint[0][1], 5, (0,0,255), -1)
            cv2.imshow("Image", frame)
            print('將物品放置在紅點處，放置後請按Q',end='\r')
        for i in range(100):
            print(f'請勿進入鏡頭範圍，目前搜尋數據量:{i+1}', end = '\r')
            frame = vision.ImageCatch(cap)
            imgBinary = vision.ImagePreProcess(frame)
...
if __name__ == '__main__':
    ...
    print(f'\nArea: Mean std Max-min Scale\n{areaMeanCont:.4f},{areaStdCont:.4f},{max(areaSizeData)-min(areaSizeData):.4f, {areaStdCont:.4f}}')
