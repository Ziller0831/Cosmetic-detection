...
def AngleIndentify(deltaX, deltaY, angle):
    ...
    if deltaX == 0: return (angle+90) if deltaY < 0 else (angle+270) if deltaY > 0 else None
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(len(contours), np.int0(fps//TargetFPS))
        dataBuffer = np.zeros((len(contours), np.int0(fps//TargetFPS), dtype=np.int8)
