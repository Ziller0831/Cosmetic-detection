...
def AngleIndentify(deltaX, deltaY, angle):
    ...
    if deltaX > 0:
        if deltaY < 0:
            ...
            return (angle + 270)   # 第四象限
    elif deltaX < 0:
        if deltaY < 0:
            return (angle + 90)    # 第二象限
        elif deltaY > 0:
            return (angle + 180)   # 第三象限
        elif deltaY == 0:
            return 180   # -X軸
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            theta2 = cv2.fastAtan2(float(deltaY), float(deltaX))
            ...
            theta1 = cv2.fastAtan2(int(vector_bb[0]), int(vector_bb[1]))
            

            
            print(f"{gravity_point}, {point1},{vector_bb},{length1:.4f},{length2:.4f}. {theta:.4f}, {theta1:.4f}")
            ...
            endPoint2 = [int(center_point[0]+ArrowLength*math.cos(math.radians(theta2))), int(center_point[1]+(-ArrowLength*math.sin(math.radians(theta2))))]
            ...
            cv2.arrowedLine(frame, center_point, endPoint2, (255,255,0), 2)
