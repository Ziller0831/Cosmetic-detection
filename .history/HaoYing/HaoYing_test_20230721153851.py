...
def WorkMode(Product):
    ...
    for i in TF_para:
        TF_para[i] = HY.XMLRead(r"E:\HaoYing\Setting.xml", str(i))
    
    frame = cv2.imread('E:\HaoYing\circle.png')
    frame = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE)

    product_features = HY.CSVDataLoad("E:\HaoYing\Cosmetic_parameter.csv", Product)
    vision = HY.EdgeDetector(2, TF_para, product_features)
    output = [0, 0, 0, 0, 0]
