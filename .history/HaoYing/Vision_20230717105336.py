...
class EdgeDetector:
    def __init__(self, ModeSW = 0,  Product_name = "", TF_parm = ""):
        ...
        if ModeSW == 0: ##% Working Mode
            Area_avg, Area_std, product_color, catch_offset, product_height =FP.CSVDataLoad(CsvDir, Product_name)
    ...
    def AngleCalc(self, minRect_P, deltaY):
        ...
        theta_a = math.degrees(math.atan2(vector_a[1], vector_b[0]))
        theta_b = math.degrees(math.atan2(vector_b[1], vector_b[0]))

        if abs(theta_a - theta_b) > 180:
            theta_b += 360
