...
class EdgeDetector:
    def __init__(self, ModeSW = 0,  ProductName = "", CsvDir = "", TransferParameters = ""):
        ...
        if ModeSW == 0: ##% Working Mode
            ...
            projection_matrix = np.dot(TransferParameters["camera_matrix"], np.hstack((rvec_matrix, TransferParameters["tvecs"])))
            ## 計算相機投影矩陣的擬合逆矩陣
    ...
    def Coordinate_TF(self, pixel_coordinate):
        ...
        homogeneous_world = np.dot(self.pseudo_inverse_projection, homogeneous2pixel)
        world_x = homogeneous_world[0] / homogeneous_world[3]
        world_x = homogeneous_world[1] / homogeneous_world[3]
 

        return world_x, world_x
