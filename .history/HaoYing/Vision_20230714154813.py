...
class EdgeDetector:
    def __init__(self, ModeSW = 0,  ProductName = "", CsvDir = "", TransferParameters = ""):
        ...
        if ModeSW == 0: ##% Working Mode
            ...
            ## 計算相機投影矩陣
            selfrvec_matrix, _ = cv2.Rodrigues(TransferParameters["rvecs"])
            projection_matrix = dot(TransferParameters["camera_matrix"], hstack((self.rvec_matrix, TransferParameters["tvecs"])))
            ## 計算相機投影矩陣的擬合逆矩陣
