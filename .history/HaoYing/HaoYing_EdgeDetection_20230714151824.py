...
if __name__ == "__main__":
    ...
    if Mode == 1:
        ...
        for i in calibration:
            FP.XMLWrite(r".\HaoYing\Setting.xml", str(i),  [[float(num) for num in row] for row in data]calibration[str(i)])
    elif Mode == 0:
        calibration = {"camera_matrix": 0, "rvecs": 0, "tvecs": 0}
        for i in calibration:
            calibration[i] = FP.XMLRead(r".\HaoYing\Setting.xml", str(i))
        print(calibration)
