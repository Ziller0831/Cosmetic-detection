...
class EdgeDetector:
    ...
    def AngleIndentify(self, deltaX, deltaY, angle):
        if deltaX > 0: return angle if deltaY < 0 else (angle+270) if deltaY == 0 else 270 if deltaY > 0 else angle
        if deltaX < 0: return (angle+90) if deltaY < 0 else (angle+180) if deltaY > 0 else (angle+180) if deltaY == 0 else 180
        if deltaX == 0: return 90 if deltaY < 0 else 270 if deltaY > 0 else 0 if deltaY == 0 else None
        # if deltaX > 0:
        #     if deltaY < 0:
        #         return angle
        #     elif deltaY == 0:
