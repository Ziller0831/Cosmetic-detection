...
##@ 影像視覺辨識
class EdgeDetector:
    def __init__(self, ModeSW = 0,  ProductName = "", CsvDir = "", TFParameters = ""):
        self.ModeSW = ModeSW
        ...
        if ModeSW == 0: ##% Working Mode
            ...
            rvec_matrix, _ = cv2.Rodrigues(TFParameters["rvecs"])
            projection_matrix = np.dot(TFParameters["camera_matrix"], np.hstack((rvec_matrix, TFParameters["tvecs"])))
