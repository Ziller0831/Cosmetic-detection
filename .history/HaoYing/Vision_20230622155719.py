...
class EdgeDetector:
    ...
    def FeaturesCalc(self, frame, contoursList):
        ...
        for contour in contoursList:
            ...
            rect = self.MinRectCircle(contour)
            centerPoint = rect[0]
            minRect = rect[1]
            rawAngle = rect[2]
            box = cv2.boxPoints(rect)

            left_point_x   = min(box[:, 0])
            right_point_x  = max(box[:, 0])
            top_point_y    = min(box[:, 1])
            bottom_point_y = max(box[:, 1])
            
            left_point_y   = box[:, 1][where(box[:, 0] == left_point_x)][0]
            right_point_y  = box[:, 1][where(box[:, 0] == right_point_x)][0]
            top_point_x    = box[:, 0][where(box[:, 1] == top_point_y)][0]
            bottom_point_x = box[:, 0][where(box[:, 1] == bottom_point_y)][0]

            vertices = array([[top_point_x, top_point_y], [bottom_point_x, bottom_point_y], [left_point_x, left_point_y], [right_point_x, right_point_y]])

            MinRectArray.append(minRect)
            if self.ModeSW != 2:
                deltaX, deltaY = vertices[1] - centerPoint
                ...
                cv2.circle(frame, centerPoint, 3, (0,255,100), -1)
                cv2.circle(frame, gravityPoint, 3, (0,0,255), -1)
