...
class initialize:
    ...
    def _ChessboardCatch(self, cap):
        ...
        while count < 10:
            ...
            key = cv2.waitKey(1)
            if  key == 13:
                cv2.imwrite(self._Chessboard_path+str(count)+'.png', frame, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
    ...
    def CameraCalibration(self, cap):
        ...
        gray = cv2.drawFrameAxes(gray, camera_matrix, dist_coeffs, rvecs, tvecs, axis_length)

        gray = cv2.circle(gray, (int(corners[0][0][0]), int(corners[0][0][1])), 5, (0, 0, 255), -1)
