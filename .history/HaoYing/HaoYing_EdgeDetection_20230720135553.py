    ...
    if Mode == 0:  ##! 物件輪廓選擇mode
        ...
        while True:
            ...
            for result in results_mean:
                pixel_coordinate = [int(result[0]),int(result[1])]
                pixel_z = int(result[2])
                object_angle = result[2]-AngleZero_offset

                text = f"({pixel_coordinate}, {object_angle:.1f})"

                cv2.putText(frame, text, (int(pixel_coordinate[0]+10),int(pixelpixel_coordinate_y+10)), cv2.FONT_HERSHEY_PLAIN,1.5,(255,64,255),2,8,0)
                world_x, world_y = vision.Coordinate_TF([pixel_x, pixel_y, 1])
                vision.Coordinate_TF()
                print(text)
