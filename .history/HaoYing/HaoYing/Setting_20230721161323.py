...
class initialize:
    def __init__(self):
        ...
        self._path = os.path.join(os.path.dirname(os.getcwd())
    ...
    def _ChessboardCatch(self, cap):
        ...
        ##@ 棋盤格截圖
        """
        _chessboard_path = os.path.join(self._path, 'CalibrationImage', 'chessboard')
        count = 0
        while count < 10:
            frame = self._vision.ImageCatch(cap)
    ...
    def _StandardCatch(self, cap):
        _standard_path = os.path.join(self._path, 'CalibrationImage' , 'standard')
        while True:
            frame = self._vision.ImageCatch(cap)
            cv2.imshow("Webcam", frame)
            print('將校正板放置在紅點處, 放置後請按Enter',end='\r')
    ...
    def CameraCalibration(self, cap):
        ...
        _standard_path = os.path.join(self._path , 'CalibrationImage' , 'standard', 'Standard.png')
