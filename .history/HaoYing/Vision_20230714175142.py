...
class EdgeDetector:
    ...
    def FeaturesCalc(self, frame, contour_list):
        ...
        for contour in contour_list:
            ...
            gravity_point = np.array(self.GPointCalc(moment))
            center_point, minRect, _ = self.MinRectCircle(contour)        
            minRect_array.append(minRect)

            if self.ModeSW != 2:

                _, deltaY = center_point - gravity_point ##! 角度計算法則在評估
                angle = self.AngleCalc(minRect, deltaY)
                
                Result_array.append([center_point[0], center_point[1], round(angle, 4)])
            
                endPoint = [int(center_point[0]+arrow_length*cos(radians(angle))), int(center_point[1]-arrow_length*sin(radians(angle)))]
                # CatchPoint = [int(center_point[0]+self.catchPoint*cos(radians(angle))), int(center_point[1]-self.catchPoint*sin(radians(angle)))]

                cv2.arrowedLine(frame, center_point, (endPoint[0], endPoint[1]), (255,100,0), 2)
                cv2.circle(frame, center_point, 5, (100,255,100), -1)
                cv2.circle(frame, gravity_point, 5, (0,0,255), -1)
