...
from numpy import array, mean, int0, hstack, dot
...
class EdgeDetector:
    def __init__(self, ModeSW = 0,  ProductName = "", CsvDir = "", TransferParameters = ""):
        ...
        if ModeSW == 0: ##% Working Mode
            ...
            ## 計算相機投影矩陣
            self.rvec_matrix, _ = cv2.Rodrigues(TransferParameters["rvecs"])
            self.projection_matrix = dot(TransferParameters["camera_matrix"], hstack((self.rvec_matrix, TransferParameters["tvecs"])))
            ## 計算相機投影矩陣的擬合逆矩陣
            pseudo_inverse_projection = linalg.pinv(projection_matrix)
