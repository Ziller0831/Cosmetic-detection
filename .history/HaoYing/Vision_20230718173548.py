...
class EdgeDetector:
    def __init__(self, ModeSW = 0,  Product_name = "", TF_parm = ""):
        ...
        if ModeSW == 2: ##% Working Mode
            ...
            self._Area_range = [_area_avg-_area_avg*5, _area_avg+_area_avg*5]
    ...
    def ContoursCalc(self, img_binary):
        ...
        contours =  [contour for contour in contours if self.Area_range[0] <= cv2.contourArea(contour) <= self.Area_range[1]]
        return contours
...
if __name__ == '__main__':
    ...
    vision = EdgeDetector(2, "長樣品瓶", CsvDir)
    while True:
