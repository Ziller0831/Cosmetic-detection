...
class EdgeDetector:
    def __init__(self, ModeSW = 1,  ProductName = "", CsvDir = ""):
        if ModeSW == 1: ## 正常狀態
            ...
            Area_Avg = product_dic.get('產品輪廓周長').get(product_index)
            Area_Std = product_dic.get('周長標準差').get(product_index)
            product_color = product_dic.get('產品顏色').get(product_index)

            self.Area_range = [length_Avg-length_Std*5, length_Avg+length_Std*5]
            self.Area_range = [300, 350]
    ...
    def ContoursCalc(self, imgBinary):
        ...
        contoursList = [contour for contour in contours if self.length_range[0] <= cv2.contourArea(contour) <= self.length_range[1]]
        return contoursList
...
if __name__ == '__main__':
    ...
    cosmetic = EdgeDetector(1, ProductName, CsvDir)
    while True:
