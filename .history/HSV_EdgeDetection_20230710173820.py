...
def MinRectCircle(contour):
    ...
    return np.int0(rawMinRect[0]), min_rect, round(rawMinRect[2], 4), rawMinRect[1][0]

def GravityCalc(moment):
    return int(moment['m10'] / moment['m00']), int(moment['m01'] / moment['m00'])

## TODO: 當質心與矩形中心重合時 None無法被round處理造成報錯
def AngleIndentify(deltaX, deltaY, angle, MinWidth, RectWidth):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            center_point, minRect, rawTheta, MinWidth = MinRectCircle(contour)
            x, y, w, h = cv2.boundingRect(contour)

            cv2.rectangle(frame, (x, y), (x + w, y + h), (200, 200, 200), 2)

            deltaX, deltaY = gravity_point - center_point
            print("X差{deltaX},deltaX Y差{deltaY}".format(deltaX, deltaY)

            # theta = cv2.fastAtan2(float(deltaY), float(deltaX))
            theta = AngleIndentify(deltaX, deltaY, 90-rawTheta, MinWidth, w)
