...
def ObjectContours(Choose_product):
    ...
    csv_path = r"C:\Users\TEST\Desktop\HaoYing0801\Data\Cosmetic_parameter.csv"
    cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
    initial = Setting.initialize()
    area_size, area_mean, area_std = initial.AreaIdent()
...
def CameraCalibration():
    ...
    for i in calibration:
        FP.XMLWrite(XML_path, str(i), [[float(num)
                    for num in row] for row in calibration[str(i)]])
    cap.release()
...
def WorkMode():
    ...
    XML_path = os.path.join(
        _path, 'Data', r'C:\Users\TEST\Desktop\HaoYing0801\Data\Setting.xml')
    for i in TF_para:
        TF_para[i] = FP.XMLRead(XML_path, str(i))

    product_path = os.path.join(
        _path, 'Data', r'C:\Users\TEST\Desktop\HaoYing0801\Data\Cosmetic_parameter.csv')
    csv_path = FP.CSVDataLoad(product_path, "長樣品蓋")
    ...
    for result in result_array:
        ...
        cv2.putText(frame, text, (int(pixel_x+10), int(pixel_y+10)),
                    cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 64, 255), 2, 8, 0)
