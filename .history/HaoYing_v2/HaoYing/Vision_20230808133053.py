...
class EdgeDetector:
    def __init__(self, ModeSW=2, TF_parm="", *Product_features):
        ...
        if ModeSW == 2:  # % Working Mode
            ...
            projection_matrix = np.dot(
                TF_parm["camera_matrix"], np.hstack((rvec_matrix, TF_parm["tvecs"])))
            ...
            self._X_offset = -204
            self._Y_offset = -95
            self._Z_offset = -_robot_height - 145

            self._Area_range = [_area_avg-_area_Std*3, _area_avg+_area_Std*3]

            if _product_color == '淺色':
                self.HSV_range = [
                    np.array([180, 25, 255]),  np.array([0, 0, 155])]
            elif _product_color == '深色':
                self.HSV_range = [
                    np.array([180, 255, 230]), np.array([0, 0, 154])]
            else:
                print('Error: No such product color')
    ...
    def ContoursCalc(self, img_binary):
        contours = cv2.findContours(
            img_binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[0]
        contours = [contour for contour in contours if self._Area_range[0]
                    <= cv2.contourArea(contour) <= self._Area_range[1]]
        return contours
        ...
        for contour in contours:
            ...
            if self._modeSW == 2:
                ...
                try:
                    ...
                    catch_p = [int(center_p[0]+self.catchPoint*math.cos(math.radians(angle))), int(
                        center_p[1]-self.catchPoint*math.sin(math.radians(angle)))]
                    cv2.arrowedLine(frame, center_p,
                                    (catch_p[0], catch_p[1]), (255, 100, 0), 2)
                    result_array.append(
                        [catch_p[0], catch_p[1], round(angle, 4)])
                except:
                    pass
