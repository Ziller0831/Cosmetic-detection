...
if __name__ == '__main__':
    ...
    while True:
        ...
        imgBinary, pixel_num = ImagePreprocess(frame)
        if pixel_num <= 1000:
            continue
        cv2.imshow("binary", imgBinary)

        contours = DeletContours(cv2.findContours(
            imgBinary, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[0])
        ...
        for contour in contours:
            ...
            if long_GC_cos < 0 and long_GC_cos >= -1 and theta <= 90 and theta >= 0:  # 第三象限反轉
                ...
                theta = cv2.fastAtan2(int(short_vect[0]), int(short_vect[1]))
                x = 1
            elif long_GC_cos > 0 and long_GC_cos <= 1 and theta >= 90 and theta <= 180:  # 第四象限反轉
                short_vect = np.array([-short_vect[0], -short_vect[1]])
                theta = cv2.fastAtan2(int(short_vect[0]), int(short_vect[1]))
                x = 2
            ...
            try:
                print(
                    f"{x}:{long_GC_cos:.2f}, {pre_theta:.2f}, {theta:.2f}, {short_vect}")

                endPoint = [int(gravity_point[0]+ArrowLength*math.cos(math.radians(theta))), int(
                    gravity_point[1]+(-ArrowLength*math.sin(math.radians(theta))))]
                cv2.arrowedLine(frame, center_point,
                                endPoint, (100, 50, 70), 2)
                cv2.arrowedLine(frame, center_point,
                                gravity_point, (255, 0, 0), 2)
                # cv2.arrowedLine(frame, gravity_point, center_point, (200,100,100), 2)
            except:
                pass
            ...
            resultData.append(
                [center_point[0], center_point[1], round(theta, 4)])
        ...
        angleData = []
        for arrayPerScan in dataBuffer:
            angleData.append([objectInArray[-1]
                             for objectInArray in arrayPerScan])
        ...
        for result in results_mean:
            # fstring
            text = f"({int(result[0])}, {int(result[1])}, {result[2]:.1f})"
            # text = "({0}, {1})".format(str(gravity_point[0]), str(gravity_point[1])) ##另一種打法
            cv2.putText(frame, text, (int(
                result[0]+50), int(result[1]+50)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, 8, 0)
        ...
        cv2.putText(frame, f"fps:{str(fps)}", (10, 30),
