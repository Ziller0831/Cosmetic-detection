...
if __name__ == '__main__':
    ...
    maxCont = max(AreaSizeData)
    minCont = min(AreaSizeData)
    meanCont = np.mean(np.array(AreaSizeData, dtype=object))
    stdCont = np.std(np.array(AreaSizeData,  dtype=float), axis=0, ddof=1)
