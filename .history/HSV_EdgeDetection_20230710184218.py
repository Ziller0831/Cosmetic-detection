...
def AngleIndentify(deltaX, deltaY, angle, diag_diff):
    ...
    if deltaX > 0:
        if deltaY < 0 & diag_diff[1] < diag_diff[0]:
            return angle, 0
        elif deltaY == 0 & diag_diff[1] < diag_diff[0]:
            return angle, 1
        else:
            return (angle + 270) % 360, 2
    elif deltaX < 0:
        if deltaY < 0:
            return (angle + 90) % 360, 3
        elif deltaY > 0:
            return (angle + 180) % 360, 4
        elif deltaY == 0:
            return (angle + 180) % 360, 5
    elif deltaX == 0:  # deltaX == 0
        if deltaY < 0:
            return (angle + 90) % 360, 6
        elif deltaY > 0:
            return (angle + 270) % 360, 7
        elif deltaY == 0:
            return angle, 8
    else:
        pass
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            theta, func = AngleIndentify(deltaX, deltaY, 90-rawTheta, abs(diff))

            print(f"X差{deltaX}, Y差{deltaY}, {abs(diff)}, {rawTheta}, {func}")
            ...
            # cv2.circle(frame, vertices[3], 3, (100,100,255), -
