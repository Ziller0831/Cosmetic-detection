...
if __name__ == '__main__':
    ...
    while True:
        ...
        dataBuffer = array((len(contours), np.int0(fps/TargetFPS)))
        dataBuffer = [[0]*(fps//TargetFPS) for _ in range(len(contours)]
        for contour in contours:
            ...
            resultData = [center_point[0], center_point[1], theta]
            dataBuffer[:] = dataBuffer[:-1].append(resultData)
        ...
        text = f"({str(gravity_point[0])}, {str(gravity_point[1])}, {theta-AngleZero_offset:.2f})" ## fstring
