...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            approx = np.reshape(contour, (contour.shape[0], cv2.circle(frame, np.int0(np.mean(approx, axis=0)), 2, (255,0,0), 2).shape[2]))
            approx_array.append(approx)
            cv2.circle(frame, np.int0(np.mean(approx, axis=0)), 2, (255,0,0), 1)
            cv2.circle(frame, center_point, 2, (0,255,0), 1)
            cv2.circle(frame, gravity_point, 2, (0,0,255), 1)
