    ...
    while True:
        ...
        for contour in contours:
            ...
            if(vect_a_val < vect_b_val):
                ...
                cv2.arrowedLine(frame, minRect[1], minRect[2], (150,200,255),4)
            elif(vect_a_val > vect_b_val):
                short_vect = np.array(vect_b)
                long_vect = np.array(vect_a)
                cv2.arrowedLine(frame, minRect[1], minRect[0], (150,200,255),4)
            
            # print(GC_vect, short_vect)

            
            # theta = AngleIndentify(GC_vect[1] ,theta)
            theta = cv2.fastAtan2(int(short_vect[0]), int(short_vect[1]))

            # _vect_dot = long_vect.dot(GC_vect)
            # _angle = _vect_dot/(np.sqrt(long_vect.dot(long_vect))*np.sqrt(GC_vect.dot(GC_vect)))
            vect_long_val = (long_vect[0]**2 + long_vect[1]**2)**0.5
            vect_GC_val = (GC_vect[0]**2 + GC_vect[1]**2)**0.5
            long_GC_cos = long_vect @ GC_vect / (vect_long_val * vect_GC_val)
            long_GC_theta = math.degrees(np.arccos(long_GC_cos))

            if long_GC_cos < 0 and long_GC_cos >= -1:   ## 反向
                theta = np.array([short_vect[0]short_vect[1]])
            elif long_GC_cos > 0 and long_GC_cos <= 1:  ## 正向
                theta = theta

            
            try:
                print(f"{long_GC_cos:.2f}, {long_GC_theta:.2f}, {theta:.2f}, {short_vect}")
                
                endPoint = [int(gravity_point[0]+ArrowLength*math.cos(math.radians(theta))), int(gravity_point[1]+(-ArrowLength*math.sin(math.radians(theta))))]
                cv2.arrowedLine(frame, endPoint, center_point, (255,0,0), 2)
