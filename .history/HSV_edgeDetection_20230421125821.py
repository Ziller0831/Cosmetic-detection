...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            gravity_point = Centroid(moment)
            center_point, minRect = MinRectCircle(contour)

            minRect_array.append(minRect)

            cv2.circle(frame, center_point, 2, (0,0,255), 2)

            cv2.circle(frame, gravity_point, 2, (0,0,255), 2)
            text = "(" + str(gravity_point[0]) + ", " + str(gravity_point[1]) + ")"
            cv2.putText(frame, text, (centroid_point[0]+10, gravity_point[1]+10), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 1, 8, 0);
