...
if __name__ == '__main__':
    ...
    while True:
        ...
        for result in results: 
            text = f"({int(result[0])}, {int(result[1])}, {result[2]-AngleZero_offset:.2f})" ## fstring
            # text = "({0}, {1})".format(str(gravity_point[0]), str(gravity_point[1])) ##另一種打法
            cv2.putText(frame, text, (int(result[0]+10), int(result[1]+10)), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 0), 2, 8, 0)
