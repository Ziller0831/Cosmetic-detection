...
def AngleIndentify(deltaX, deltaY, angle):
    ...
    if deltaX > 0:
        if deltaY < 0:
            ...
            return (angle + 270) % 360, 2   # 第四象限
    elif deltaX < 0:
        if deltaY < 0:
            return (angle + 90) % 360, 3    # 第二象限
        elif deltaY > 0:
            return (angle + 180) % 360, 4   # 第二象限
        elif deltaY == 0:
            return (angle + 180) % 360, 5   # 第二象限
    elif deltaX == 0:  # deltaX == 0
        if deltaY < 0:
            return (angle + 90) % 360, 6    # 第二象限
        elif deltaY > 0:
            return (angle + 270) % 360, 7   # 第二象限
        elif deltaY == 0:
            return angle, 8
    else:
        pass
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            print(f"X差{deltaX}, Y差{deltaY}, {diff}, rawTheta:{90-rawTheta:.2f}, theta:{theta:.2f}, {func}, {func2}")
            ...
            # cv2.arrowedLine(frame, vertices[0], vertices[1], (255,100,0), 2)
