...
if __name__ == '__main__':
    ...
    while True:
        ...
        for contour in contours:
            ...
            pre_theta = theta
            x = 0
            if long_GC_cos < 0 and long_GC_cos >= -1 and theta < 90 or theta > 359.9:   ## 反向
                short_vect = np.array([-short_vect[0], -short_vect[1]])
                theta = cv2.fastAtan2(int(short_vect[0]), int(short_vect[1]))
                x = 1
            elif long_GC_cos < 0 and long_GC_cos >= -1 and theta < 90 or theta > 359.9:  ## 正向
                x = 2
            
            
            
            try:
                print(f"{x}:{long_GC_cos:.2f}, {pre_theta:.2f}, {theta:.2f}")
