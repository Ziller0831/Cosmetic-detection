...
ProductName = "長樣品瓶"  # Input from UI
CsvDir = r"./Cosmetic_parameter.csv"

class EdgeDetector:
    def __init__(self, ModeSW = 0,  ProductName = "", CsvDir = ""):
        if ModeSW == 0: ## 正常狀態
        ...
        else if ModeSW == 1: ## EdgeAreaDetect mode
            self.Area_range = [500, 5000]
            self.HSV_range = [np.array([180, 25, 255]), np.array([0, 0, 155])]
        else if ModeSW == 2: ## Calibration mode
            self.Area_range = [100, 10000]
            self.HSV_range = [np.array([180, 25, 255]), np.array([0, 0, 155])]
    ...
    def FeaturesCalc(self, frame, contoursList):
        ...
        for contour in contoursList:
            ...
            CatchPoint = [int(centerPoint[0]+ArrowLength*cos(radians(angle))), int(centerPoint[1]+(-ArrowLength*sin(radians(angle))))]
...
if __name__ == '__main__':
    ...
    cosmetic = EdgeDetector(0, ProductName, CsvDir)
    while True:
