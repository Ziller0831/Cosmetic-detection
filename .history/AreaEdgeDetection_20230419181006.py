...
Contour_area = [3000, 6000]


def nothing(x):
    pass

# def delet_contours(moment):
#     contoursList = []
...
def draw_min_rect_circle(contours):
    ...
    for cnt in contours:
        ...
        x_means = np.mean(min_rect[:, 0])
        y_means = np.mean(min_rect[:, 1])
...
while 1:
    ...
    for contour in RawContours:
        try:
            ...
            # contours = delet_contours(moment)
    ...
    min_rect_array = draw_min_rect_circle(RawContours)

    cv2.drawContours(frame, RawContours, -1, (0,0,255), 2)
