    ...
    if Mode == 1: ##! 初始化模式
        ...
        while True:
            ...
            for result in results_mean:
                ...
                cv2.putText(frame, text, (int(result[0]+10), int(result[1]+10)), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 64, 255), 
            
            fps = round(1/(time() - time_start), 2)
            cv2.putText(frame, f"fps:{str(fps)}", (10, 30), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 2)
            cv2.imshow("Result", frame)

            # print(f"料件判斷{object}",end="\r")
            # try:
            #     print(f"料件座標:x{x_coordinate[0]}.y{y_coordinate[0]}",end="\r")
            # except:
            #     pass
            time_start = time()
