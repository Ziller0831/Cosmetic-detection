...
視覺辨識拆分為3個功能
    ...
    CameraCalibration -> 相機校正與對Robot標定
    WorkMode_loop     -> 視覺辨識
'''
...
def ObjectContours(object):
    cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)    # Image cat
    initial = Setting.initialize(object)
    _, areaMean, areaStd = initial.AreaIdent()
...
def CameraCalibration():
    ...
    [cameraMatrix, rvecs, tvecs] = initial.CameraCalibration(cap)
