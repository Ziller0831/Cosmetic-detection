...
def WorkMode_loop(Object):
    # UDP setup
    target_ip = '127.0.0.1'
    Letview_port = 65235 ##* Letview UDP port
    python_port = 65234
        ...
        try:
            ...
            try:
                ...
                for result in featuresArray:
                    ...
                    resultData = f"({Output[3]:.1f})"
                    cv2.putText(frame, resultData, (int(pixel_x+10),int(pixel_y+10)), cv2.FONT_HERSHEY_PLAIN,1.5,(255,64,255),2,8,0)

                    # 將訊息透過UDP發送到目標
                    text = str(Output)
                    UDP_socket.sendto(text.encode(), (target_ip, Letview_port))
