...
架構：
    ...
    |
    +-- def CSVDataLoad(CSV路徑, 物件名稱)
    |       return 輪廓面積平均, 輪廓面積標準差, 物件顏色, 抓取點偏移, 物件高度
    |
    +-- def XMLWrite(XML路徑, 索引標籤, 放入的資料)
    |
    +-- def XMLRead(XML路徑, *索引標籤)
        |       return 相機內外參矩陣, 旋轉向量, 平移向量
'''
...
##@ Rewrite .xml data
def XMLWrite(xmlPath, tag, message):
    tree = XET.parse(xmlPath)
    root = tree.getroot()
