...
def CSVDataLoad(csvPath="", productName=""):
    ...
    product_index  = next((key for key,name in productDic.get('產品名稱').items() if name == productName), None)

    area_avg       = productDic.get('面積平均').get(product_index)
    area_std       = productDic.get('面積標準差').get(product_index)
    product_color  = productDic.get('產品顏色').get(product_index)
    catch_offset   = productDic.get('吸取點').get(product_index)
    product_height = productDic.get('物件橫躺高度').get(product_index)
