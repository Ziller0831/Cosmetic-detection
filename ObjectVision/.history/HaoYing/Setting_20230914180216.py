...
class initialize:
    ...
    def CameraCalibration(self, cap):
        ...
        ret, cameraMatrix, dist_coeffs, rvecs, tvecs = cv2.calibrateCamera(objPoints, imgPoints, gray.shape[::-1], None, None)
        ...
        stdImg = cv2.drawFrameAxes(std_img, camera_matrix, dist_coeffs, rvecs, tvecs, axis_length)

        stdImg = cv2.circle(std_img, (int(corners[0][0][0]), int(corners[0][0][1])), 5, (0, 0, 255), -1)
