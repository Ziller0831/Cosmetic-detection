...
class initialize:
    ...
    def CameraCalibration(self, cap):
        ...
        _, rvecs, tvecs = cv2.solvePnP(objp, corners,         print(f"Camera inner parameter: {cameraMatrix}")
, distCoeffs)

        axis_length = self._Chessboard_squareSize * 2  # 座標軸的長度
        stdImg = cv2.drawFrameAxes(std_img, cameraMatrix, distCoeffs, rvecs, tvecs, axis_length)
