...
def CSVDataLoad(csvPath="", productName=""):
    ...
    area_avg       = productDic.get('面積平均').get(productIndex)
    area_std       = productDic.get('面積標準差').get(productIndex)
    product_color  = productDic.get('產品顏色').get(productIndex)
    catch_offset   = productDic.get('吸取點').get(productIndex)
    product_height = productDic.get('物件橫躺高度').get(productIndex)
