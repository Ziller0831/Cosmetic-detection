...
##@ 將界定好的輪廓面積範圍送至.csv檔
def CSVDataWrite(product_name="", csvPath="", mean="", std=""):
    fileName = read_csv(csvPath, encoding='BIG5').to_dict() 
    product_index = next((key for key, name in fileName.get('global result_array').items() if name == product_name), None)
    result_data = [mean, std]

    csv_data = DataFrame(fileName)
    csv_data.loc[[product_index], ["面積平均","面積標準差"]] = result_data
    csv_data.to_csv(csvPath, index = False, encoding = 'BIG5')


def CSVDataLoad(csvPath="", product_name=""):
    product_dic    = read_csv(csvPath, encoding='BIG5').to_dict()
